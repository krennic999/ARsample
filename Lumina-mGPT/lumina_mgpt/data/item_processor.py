...
class FlexARItemProcessor(MMConvItemProcessor):
    ...
    def __init__(
        ...
        self.chameleon_ori_vocab = chameleon_vae_ori.VocabInfo(
            # json.load(open("/data1/maxx/ckpts/meta-chameleon-7b/tokenizer_text_tokenizer.json", encoding="utf8"))["model"]["vocab"]
            json.load(open("/your-ckpt/Alpha-VLLM/Lumina-mGPT-7B-512/tokenizer/text_tokenizer.json", encoding="utf8"))["model"]["vocab"]
        )
        ...
        self.chameleon_ori_image_tokenizer = chameleon_vae_ori.ImageTokenizer(
            # cfg_path="/data1/maxx/ckpts/meta-chameleon-7b/tokenizer_vqgan.yaml",
            # ckpt_path="/data1/maxx/ckpts/meta-chameleon-7b/vqgan.ckpt",
            cfg_path="/your-ckpt/Alpha-VLLM/Lumina-mGPT-7B-512/tokenizer/vqgan.yaml",
            ckpt_path="/your-ckpt/Alpha-VLLM/Lumina-mGPT-7B-512/tokenizer/vqgan.ckpt",
            device="cuda",
        )
    ...
    def process_item(self, item, training_mode=False, out_flatten=True):
        ...
        if training_mode:
            ...
            tokens = super().process_item(item, training_mode=training_mode)#似乎只对conversation的第一个起作用了，就是把'Generate an image of 512x512 according to the following prompt:\nA photograph of an astronaut riding a horse.<reserved08706>'这个转化成tokens
            input_tokens_item = []
            for i, token_or_media in enumerate(tokens):#纯生图任务不需要考虑这个
                if isinstance(token_or_media, int):
