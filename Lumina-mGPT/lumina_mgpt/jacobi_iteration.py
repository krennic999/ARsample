...
def prefix_matching_next_tokens(
    ...
    is_prefilling_phase=False,#当(output_token_num <= 1)时is_prefilling_phase=true
    ...
    **kwargs,
    ...
    if is_prefilling_phase:#当is_prefilling_phase时，只预测出一个token，所以
...
def get_multi_token_for_preparation(
    ...
    **kwargs,
    ...
    if multi_token_init_scheme == 'random':#始终等random
        ...
        rand_tokens = img_vocab[rand_tokens]#随机生成rand_token_num个rand token
    ...
    else:#这个看起来是根据之前的tokens初始化新token的代码
        # 看不懂，先放一边
        img_vocab = img_vocab.to(device)
        img_vocab_size = len(img_vocab)
        rand_tokens = torch.randint(
        ...
        if (img_width > 0) and (input_ids_len + additional_tokens_len >= prefill_num) and (rand_token_num > 0):
            ...
            if 'sample' in multi_token_init_scheme:
                # 根据last_resampled_input_logits得到top1的embedding和conf
    ...
    class JacobiSampler(model_class, nn.Module):
        ...
        def prepare_inputs_for_generation_jacobi(
            ...
            **kwargs,
            ...
            all_collected_length = input_ids.shape[1]#当前sequence长度
            is_append_random_tokens = kwargs.get("is_append_random_tokens", False) ###!!!#始终true
            ...
            temporary_collected_scores = kwargs.get("temporary_collected_scores", None)#当前input id对应的one-hot tensor

            generator = kwargs.get("generator", None)
            input_ids_accept_len_ptr = kwargs.get("input_ids_accept_len_ptr", 0)

            if is_append_random_tokens:#始终为true
                
                if additional_tokens is not None:
                    # 计算要新加入的random tokens
                ...
                # 随机生成rand_token_num个新的rand token
                # 或者根据horizon啥的初始化
                # 如果用的是rand初始化，返回的score就是token-idx对应的one-hot tensor
                ...
                if additional_tokens is not None:
                    # additional_tokens是啥，为啥rand token数量由它决定
                    # 把当前token和新加的rand tokens合到一起得到新的输入序列
                    input_ids = torch.cat([ 
                        input_ids, 
                    ...
                    # 如果additional_tokens是None的话新的sequence就只有input_ids和rand_tokens
                    input_ids = torch.cat([ 
                        input_ids, 
                        rand_tokens,
                    ], dim=-1)
            ...
            if past_key_values is not None:#cache_position切分数据，只使用尚未处理的部分
                ...
                # 从tensor_list中根据cache_position按dim=1收集数据，并将这些数据拼接在一起
                # all_collected_length：input ids的总长
                input_token_scores = gather_from_split_tensors(
                    tensor_list = [
                        temporary_collected_scores[:, -1:], #上一个sequence最后一个token
                        recycled_scores, #recycled_scores不知道啥意思，应该是jacobi迭代过程中还没有接收的token
                        scores_of_rand_tokens,#新加入的rand token
                    ],
            ...
            if attention_mask is not None and position_ids is None:
                ...
                position_ids.masked_fill_(attention_mask == 0, 1)#这样和zipar是一致的了，uncond部分text默认填充1

                if len(position_ids.shape) == 3: ###!!!
                    # 目前暂时还不知道position_ids的第二个维度是啥东西
            ...
            **kwargs,
            ...
            if use_chameleon_tokenizer:
                ...
                chameleon_ori_vocab = chameleon_vae_ori.VocabInfo(
                    json.load(open("/your-ckpt/Alpha-VLLM/Lumina-mGPT-7B-512/tokenizer/text_tokenizer.json"))["model"]["vocab"]
                )
        ...
        def _sample(#default decoding in GenerationMixin
            self,
            ...
            **model_kwargs,
            ...
            # 在GenerationMixin中会把logits_processor和input_id传进来，decode得到分数
            # 这个就是整个生成过程
            r"""
            ...
            unfinished_sequences = torch.ones(batch_size, dtype=torch.long, device=input_ids.device)#标记每个序列是否完成
            
            # 这两句是新加的
            temporary_collected_scores = input_ids.new_zeros((batch_size, cur_len, self.config.vocab_size))#BLV
            # 构造一个one-hot tensor，id对应的位置值为1
            ...
            prefill_num = model_kwargs['attention_mask'].shape[1] - 1#cache中缓存的token个数
            ...
            if do_cfg:
                #attention_mask会加倍，为了准备cfg，uncond部分的mask是0
                # 但是zipar里是通过修改文本部分的input_id=1得到的undond
            ...
            # jacobi_loop_interval_l：模型从 左侧 起始点开始进行并行生成计算的区间
            # jacobi_loop_interval_r：从 右侧 起始点开始进行并行生成计算的区间。r 通常表示区间的 结束
            jacobi_loop_interval_lr = (cur_len + self.jacobi_loop_interval_l, cur_len + self.jacobi_loop_interval_r)
            gen_loop_num = 0
            max_num_new_tokens = self.max_num_new_tokens#始终为16

            device = input_ids.device
            dtype = input_ids.dtype

            if self.prefix_token_sampler_scheme == 'speculative_jacobi':#默认是这个
            ...
            while self._has_unfinished_sequences(
                ...
                # soi：8197；eoi：8196；eol：8803；hw indicator：h/w*2+8804
                ...
                # 如果 image_start_token_id 或 image_end_token_id 为空，则返回 False，
                # 如果这两个值都不为空，函数会继续执行其他逻辑（此处没有展示），以决定是否使用 CFG
                ...
                # 看起来就是普通的从logit到token的过程
                next_tokens, next_token_scores = sampling_logits2tokens(
                    logits,
                    all_collected_input_ids,
                    unfinished_sequences, pad_token_id,
                    output_token_num = output_token_num,#当前步生成的token数量
