...
def visual_and_save_attention(attn_map, layer_idx):
    # torch.Size([1, 32, 4203, 4203])
    heads = attn_map.shape[1]
    
    for i in range(heads):
        # 转换为 float32 并移动到 CPU
        test_ = attn_map[0, i].to(torch.float32).cpu().numpy()

        # 设定 colorbar 和 heatmap 显示范围为 [1e-4, 1]
        vmin = 1e-4  # 最小值
        vmax = 1      # 最大值

        # 使用 LogNorm 来为 colorbar 设置对数刻度
        norm = colors.LogNorm(vmin=vmin, vmax=vmax)

        # 绘制热力图，不进行对数缩放，直接显示数据
        cax = plt.imshow(test_, cmap='viridis', norm=norm)

        # 添加对数尺度的 colorbar，设置刻度范围为 [1e-4, 1]
        cbar = plt.colorbar(cax)
        cbar.set_ticks([1e-4, 1e-3, 1e-2, 1e-1, 1])  # 对数刻度的刻度值
        cbar.set_ticklabels([r'$10^{-4}$', r'$10^{-3}$', r'$10^{-2}$', r'$10^{-1}$', r'$1$'])  # 对应的标签

        # 保存图片
        if not os.path.exists('./attn_maps'):os.makedirs('./attn_maps')
        plt.savefig(f'./attn_maps/layer{layer_idx}_head_{i}.png')
        plt.close()
        print(f'Layer {layer_idx}, Head {i} saved...')

    # 转换为 float32 并移动到 CPU
    test_ = torch.mean(attn_map[0],dim=0,keepdim=False).to(torch.float32).cpu().numpy()
    ...
    # 保存图片
    if not os.path.exists('./attn_maps'):os.makedirs('./attn_maps')
    plt.savefig(f'./attn_maps/layer{layer_idx}_avg.png')
    plt.close()
...
class ChameleonModel(ChameleonPreTrainedModel):
    ...
    def forward(
        ...
        for layer_idx,decoder_layer in enumerate(self.layers):
            ...
            if output_attentions:
                ...
                # visual_and_save_attention(layer_outputs[1],layer_idx)
    ...
    def _update_causal_mask(
        ...
        past_seen_tokens = past_key_values.get_seq_length() if past_key_values is not None else 0#训练的时候是0
        using_static_cache = isinstance(past_key_values, StaticCache)#训练时False
        ...
        if attention_mask is not None and attention_mask.dim() == 4:
            ...
            causal_mask = causal_mask[None, None, :, :].expand(input_tensor.shape[0], 1, -1, -1)#1,1,sequence_length,sequence_length+1
...
class ChameleonForConditionalGeneration(ChameleonPreTrainedModel):
    ...
    def forward(
        ...
        input_ids: torch.LongTensor = None,
        pixel_values: torch.FloatTensor = None,
        attention_mask: Optional[torch.Tensor] = None,#在这层里attention_mask是None
        position_ids: Optional[torch.LongTensor] = None,
        past_key_values: Optional[Cache] = None,
        inputs_embeds: Optional[torch.FloatTensor] = None,
