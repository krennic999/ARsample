...
class MMConvItemProcessor(ItemProcessorBase):
    ...
    def add_speaker_and_signal(self, source: List):
        ...
        for i, sentence in enumerate(source):
            ...
            conv.append_message(role, value)#句子后面加了<reserved08706>
    ...
    def process_item(self, data_item: dict, training_mode=False) -> Tuple[List, List]:
        data_item = self.preprocess_item(data_item)#啥操作也没执行

        d_media = self.collect_and_process_media(data_item)#返回的d_media应该是包含image的tensors的json文件
        ...
        if not self.implicit_at_beginning:
            source = self.insert_implicit_media_symbol_in_q1(source, d_media)#这个是检测对话中是否有<|image|>，没有的话就插在最前面，d_media里啥也没有的话就不起作用

        conversation, pieces = self.add_speaker_and_signal(source)

        if self.implicit_at_beginning:
            # conversation: 'Generate an image of 512x512 according to the following prompt:\nA photograph of an astronaut riding a horse.<reserved08706>'
        ...
        for media_symbol, l_media in d_media.items():
            media_token = self.d_media_symbol2token[media_symbol]#self.d_media_symbol2token：{'<|image|>': 65536}
            media_token_count = tokens.count(media_token)#对于生成任务，count=0
        ...
        tokens, labels = self.replace_media_token_with_media(tokens, labels, d_media)#对于生成任务不起作用
