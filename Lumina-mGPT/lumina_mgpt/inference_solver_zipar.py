...
class FlexARInferenceSolver:
    ...
    def __init__(self, model_path, precision, target_size=512):
        ...
        self.image_start_token_id = self.item_processor.token2id(self.item_processor.image_start_token)
        self.image_end_token_id = self.item_processor.token2id(self.item_processor.image_end_token)
    ...
    def decode_ids(self, tokens: List[int]):
        ...
        while i < len(tokens):
            token_id = tokens[i]
            # if token_id == self.item_processor.token2id(self.item_processor.image_start_token): ## modified here
            if token_id == self.image_start_token_id:
                cache = []
                for j in range(i + 1, len(tokens)):
                    # if tokens[j] != self.item_processor.token2id(self.item_processor.image_end_token):
                    if tokens[j] != self.image_end_token_id:
