...
def check_eol_in_multitokens(tokenlen, new_pred_tokenlen, line_len):
    # 在处理多 token（可能是多行的文本生成任务）时，检查在一个特定的行宽 line_len 下，是否会跨越行尾（end-of-line, EOL）
    # tokenlen：所有token总长度；new_pred_tokenlen：即将添加的新token长；line_len：当前行最多能容纳多少token
...
def get_eol_in_multitokens(logits, eol_cls, tokenlen, new_pred_tokenlen, line_len, min_dtype=-math.inf):
    # 当检测到跨行时，修改 logits 以强制插入行尾符（EOL）的位置
...
class MultiTokensVLLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        if self.num_image_start_tokens == self.num_image_end_tokens:
            ...
            # 现在score是一个dim=3的tensor，1，1，65536，不知道三个维度分别表示啥
            # new_logit_token_len这个推测可能是模型在一个step中添加的token的数量，被划分到一个新的dim，需要和sample结合起来看
            ...
            if new_token_num >= 2:
                ...
                tokens = input_ids[0][self.image_start_token_id_index + 3 :]#去掉soi和hw indicator后的有效token数
                ...
                )#检测加上这些token后是否会到eol，没有跨越就返回true
                ...
                )#检测加上这些token后是否会到eoi，没有跨越就返回true

                # TODO: is_pre_seq_containing_end_of_img:
                scores = torch.where(#暂时不明白这个suppress_token_mask和suppress_token是啥意思，可能也是关于特殊token的
