...
from diffusers.models.normalization import AdaLayerNormContinuous, AdaLayerNormZeroSingle, GlobalResponseNorm, RMSNorm
...
class AdaLayerNormZero(nn.Module):
    ...
    def forward(
        ...
        if self.emb is not None:#在text condition作为输入时，cond还是对齐的，经过这个norm后就不对了 ;emb是空
...
class TransformerBlock(nn.Module):
    ...
    def forward(
        ...
        #c_gate_msa不一样！c_scale_mlp也不一样;c_gate_mlp也不一样，why
...
class Transformer2DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def forward(
        ...
        try:
            ...
            print(micro_cond_embeds.shape,pooled_projections.shape)
        ...
        for index_block, block in enumerate(self.transformer_blocks):
            ...
            else:#这步输入的hidden_states，encoder_hidden_states啥的都是完全一样的
        ...
        hidden_states = torch.cat([encoder_hidden_states, hidden_states], dim=1)#到这里cfg前和cfg后就不一样了,encoder_hidden_states和hidden_states都不一样
