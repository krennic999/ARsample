...
class Pipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        entropy_range=None,temperature_value=None
    ):
        """
        ...
        logits_output=None
        entropy_mean_list=[]
        prev_masking=torch.ones(batch_size, height//16 * width//16,device=latents.device)
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, timestep in enumerate(self.scheduler.timesteps):
                ...
                # print("Scheduler class:", self.scheduler.__class__)
                latents_,masking = self.scheduler.step(
                    ...
                    generator=generator,
                    cur_step=i,
                )#masking=False的地方是本次要留下来的，呃但是我要得到这个token第一次被接受的时候
                latents=latents_.prev_sample
                probs = model_output
                masking=masking.view(probs.shape[0],1,probs.shape[2],probs.shape[3]).float().to(latents.device)
                prev_masking=prev_masking.view(probs.shape[0],1,probs.shape[2],probs.shape[3]).float()
                if masking!=None:
                    if logits_output!=None:
                        logits_output=logits_output+probs*(prev_masking-masking)
                    else:
                        logits_output=torch.zeros_like(probs)+probs*(prev_masking-masking)
                        
                probs_cur=F.softmax(probs,dim=1)*(prev_masking-masking)
                entropy_cur=(- (probs_cur * torch.log(probs_cur + 1e-12)).sum(dim=1,keepdim=True))*(prev_masking-masking)
                entropy_mean_list.append((entropy_cur.sum()/((prev_masking-masking).sum())).item())
                prev_masking=masking.view(batch_size, height//16 * width//16)
        ...
        if return_entropy:
            return ImagePipelineOutput(output),logits_output, entropy_mean_list
        else:
