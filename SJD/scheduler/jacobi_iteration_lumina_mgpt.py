...
class SpeculativeSampler:
    ...
    def __init__(
        ...
        self.if_entropy_acc=if_entropy_acc
    ...
    def __call__(
        self, model_type,
        draft_tokens, advanced_tokens, draft_prob, advanced_prob,
        logits_processor = None, logits_warper = None,
        all_collected_input_ids = None,
        **kwargs,
    ): 
        ...
        if self.if_entropy_acc:
            mask = torch.rand_like(rs) < 0.2    #0.0是llamagen，0.2是mgpt
        ...
        for b in range(B):
            ...
            for i in range(1, L):
                ...
                target_token_index = advanced_token_index_selector(i)#target_token_idx+1和draft_token_idx对应相同的token
                ...
                sampled_advanced_entropy = - (advanced_prob[:, target_token_index,:] * torch.log(advanced_prob[:, target_token_index,:] + 1e-12)).sum(dim=-1)

                r = rs[b, i, cls_idx]
                # print(self.if_entropy_acc)
                if self.if_entropy_acc:
                    if model_type=='lumina':
                        entropy_scale = ((sampled_advanced_entropy / 9)+0.05).clamp(0, 1)  # shape: (B, I, 1) 或 (B, I)
                        r = (r * entropy_scale).clamp_(0, 1)

                    else:
                        r = (rs[b, i, cls_idx]*(sampled_advanced_entropy/8)).clamp_(0,1)#ours
                        r = (rs[b, i, cls_idx]*0.5).clamp_(0,1)#ab1

                self.sampling_last_draft_token[b] = cls_idx

                if (r <= (sampled_advanced_prob / sampled_draft_prob).clamp(max=1)):# or sampled_draft_entropy<1.0: 
                    ...
                    # 用advanced-draft的prob，归一化一下作为新的prob
    ...
    if is_prefilling_phase:
        ...
        if prefix_token_sampler is not None:
            ...
            first_misaligned_input_token_inds, next_tokens, next_token_scores = prefix_token_sampler(
                ...
                draft_prob = input_token_scores,#上一个jacobian step的probs(b,l,V),l可能是window
                advanced_prob = next_token_scores,#新的jacobian step的probs(b,l,v)
                **kwargs,
            )
...
def push_forward_model_kwargs_and_inputs(
    ...
    # additional_tokens是当前step中没有被接受的tokens
...
def get_multi_token_for_preparation(#生成rand_token_num个新的token，贴到
    img_vocab, 
    rand_token_num, 
    ...
    **kwargs,
    ...
    if multi_token_init_scheme == 'random':
        ...
        rand_tokens = img_vocab[rand_tokens]#rand_tokens是[b,rand_token_num]
        ...
        # print(rand_token_num)
        rand_tokens = img_vocab[rand_tokens]
        # temporary_collected_scores:[b,l,V], 生成的scores_of_rand_tokens是[b,rand_token_num,V]
        scores_of_rand_tokens = temporary_collected_scores.new_zeros(
            (*temporary_collected_scores.shape[:-2], rand_token_num, temporary_collected_scores.shape[-1])
        )
        scores_of_rand_tokens = torch.scatter(scores_of_rand_tokens, -1, rand_tokens.unsqueeze(-1), 1.0)
        img_width = kwargs.get("img_width", None)
        pad_len = 1 if model_type=='lumina' else 0
        ...
        if (img_width > 0) and (input_ids_len + additional_tokens_len >= prefill_num) and (rand_token_num > 0):
            ...
            # last_flatten_indices=torch.ones_like(last_flatten_indices)*(input_ids.shape[1]-1)
    ...
    class JacobiSampler(model_class, nn.Module):
            ...
            **kwargs,
            ...
            if is_append_random_tokens:
                ...
                rand_tokens, scores_of_rand_tokens = get_multi_token_for_preparation(#
                    ...
                    img_width = kwargs.get("img_width", None),
                    model_type = self.model_type_2,
                    generator = generator,
                    prefill_num = prefill_num,
                )
                if additional_tokens is not None:
                    input_ids = torch.cat([
                        input_ids, #上一个step的输出，作为新的step的第一个输入
                        additional_tokens[:, : additional_tokens.shape[1] + kept_unconverged_token_num], #之前的window里没被接受的tokens
                        rand_tokens,#新生成的token
                    ], dim=-1)
                else:
                    input_ids = torch.cat([ 
                        input_ids, 
            ...
            if past_key_values is not None:
                ...
                # TODO:temporary_collected_scores，recycled_scores分别是啥
                # 这个函数啥意思啊
        ...
        def _init_new_params(
            ...
            **kwargs,
            ...
            self.save_NFE_dir=save_NFE_dir
        ...
        def _sample(
            ...
            **model_kwargs,
            ...
            # temporary_collected_scores是根据input_ids转化成one-hot得到的
            ...
            if self.prefix_token_sampler_scheme == 'speculative_jacobi':
                prefix_token_sampler = SpeculativeSampler(
                    generator=self.generator,
                    if_entropy_acc=self.if_entropy_acc,
            ...
            while self._has_unfinished_sequences(
                ...
                matched_next_scores, unmatched_next_scores = prefix_matching_next_tokens(
                    model_type=self.model_type_2,
                    model_input_ids=model_input_ids, #上一个jacobi step的model_input_ids
                    input_token_scores = input_token_scores,
                    next_tokens=next_tokens, #新的jacobi step
                ...
                output_token_num = min(max_num_new_tokens, jacobi_loop_interval_lr[-1] - cur_len) if (
                    cur_len >= jacobi_loop_interval_lr[0]#在生成图像时一次输出多个token，生成生成前三个token时只输出一个
            ...
            if count_time:
                ...
                with open(self.save_NFE_dir,'a') as f:
                    f.write('NFE:%d, latency: %.2f\n'%(gen_loop_num,t))
