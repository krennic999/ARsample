...
model_path='/your-ckpt/Alpha-VLLM/Lumina-mGPT-7B-1024'
target_size = 1024
target_size_h, target_size_w = 1024, 1024
...
multi_token_init_scheme = 'random'#'sample_horizon'#'sample_horizon'#'random'#
is_entropy_acc=True#True
image_top_k = 2000 
text_top_k = 10
guidance_scale = 3.0
prefix_token_sampler_scheme = 'speculative_jacobi' # 'jacobi', 'speculative_jacobi'
save_NFE_dir='test_lumina3_nfe_%s_entropy%s.txt'%(multi_token_init_scheme,is_entropy_acc)
savedir_root="./workdir_new3_%s_entropy%s/"%(multi_token_init_scheme,is_entropy_acc)
...
for seed in seeds:
    ...
    for i, q_image_content_condition in enumerate(q_image_content_conditions):
        ...
        with open(save_NFE_dir,'a') as f:
            f.write('Prompt: %s '%q_image_content_condition)
        ...
        output_file_name = model_path.split("/")[-1] + "-" + q_image_content_condition[:30] + '-' + str(max_num_new_tokens) + '-init-sample' '-seed' + str(seed) + '-img_topk' + str(image_top_k) + ".png"
        ...
        result_image.save(savedir_root + output_file_name)
        # os.makedirs("./workdir_768_2/",exist_ok=True)
        # result_image = inference_solver.create_image_grid([new_image], 1, 1)
        # result_image.save("./workdir_768_2/" + output_file_name)
        print(a1, 'saved', savedir_root + output_file_name) # <|image|>
